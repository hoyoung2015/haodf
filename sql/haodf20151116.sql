...
-- ----------------------------
-- Table structure for zixun_wenda
-- ----------------------------
DROP TABLE IF EXISTS `zixun_wenda`;
CREATE TABLE `zixun_wenda` (
  `zw_id` int(11) NOT NULL AUTO_INCREMENT,
  `zixun_id` varchar(65) DEFAULT NULL,
  `zw_type` varchar(255) DEFAULT NULL,
  `create_time` datetime DEFAULT NULL,
...
-- ----------------------------
-- View structure for doc_hos
-- ----------------------------
DROP VIEW IF EXISTS `doc_hos`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER  VIEW `doc_hos` AS SELECT *,TO_DAYS(NOW())-TO_DAYS(start_time),total_view/(TO_DAYS(NOW())-TO_DAYS(start_time)) as day_view from doctor ;
